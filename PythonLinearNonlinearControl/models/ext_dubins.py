...
class ExtDubinsModel(Model):
    ...
    def __init__(self, config):
        """
        """
        super(ExtDubinsModel, self).__init__()
        self.dt = config.DT
    ...
    def predict_next_state(self, curr_x, u):
        ...
        if len(u.shape) == 1:
            ...
            x_dot = np.array([curr_x[2]*np.cos(curr_x[3]), curr_x[2]*np.sin(curr_x[3]), u[0], u[1]])

            # B = np.array([[curr_x[-2]*np.cos(curr_x[-1]), 0., 0.],
            #               [curr_x[-2]*np.sin(curr_x[-1]), 0., 0.],
            #               [0., 1., 0.],
            #               [0., 0., 1.]])
            # calc dot
            ...
            # insert
            x_pop = np.multiply(curr_x[:,2], np.cos(curr_x[:,3]))
            y_pop = np.multiply(curr_x[:,2], np.sin(curr_x[:,3]))
            v_pop = u[:,0]
            theta_pop = u[:,1]
            x_dot = np.zeros((pop_size, state_size))
            x_dot[:,0] = x_pop
            x_dot[:,1] = y_pop
            x_dot[:,2] = v_pop
            x_dot[:,3] = theta_pop
    ...
    def predict_adjoint_state(self, lam, x, u, g_x=None, t=None):
        ...
        Returns:
            prev_lam (numpy.ndarrya): previous adjoint state,
                shape(state_size, )
        """
        return super().predict_adjoint_state(lam, x, u, g_x, t)
        # if len(u.shape) == 1:
        #     delta_lam = self.dt * \
    ...
    def calc_f_x(xs, us, dt):
        ...
        f_x[:,0,2] = np.cos(xs[:,3])
        f_x[:,0,3] = -xs[:,2]*np.sin(xs[:,3])
        f_x[:,1,2] = np.sin(xs[:,3])
        f_x[:,1,3] = xs[:,2]*np.cos(xs[:,3])
        # f_x[:, 0, 2] = -np.sin(xs[:, 2]) * us[:, 0] * xs[:,3]
    ...
    def calc_f_u(xs, us, dt):
        ...
        f_u[:,2,0] = np.ones_like(f_u[:,2,0])
        f_u[:,3,1] = np.ones_like(f_u[:,3,1])
    ...
    def calc_f_xx(xs, us, dt):
        ...
        f_xx[:,0,2,3] = -np.sin(xs[:,3])
        f_xx[:,0,3,2] = -np.sin(xs[:,3])
        f_xx[:,0,3,3] = -xs[:,2]*np.cos(xs[:,3])

        f_xx[:,1,2,3] = np.cos(xs[:,3])
        f_xx[:,1,3,2] = np.cos(xs[:,3])
        f_xx[:,1,3,3] = -xs[:,2]*np.sin(xs[:,3])
